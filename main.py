...
class UsernameChecker:
    def __init__(self):
        ...
        print(f"Total possible usernames: {len(self.charset) ** USERNAME_LENGTH:,}")
    ...
    async def check_username(self, username: str):
        for attempt in range(MAX_RETRIES + 1):
            try:
                async with self.session.post(API_URL, json={"username": username}, timeout=10) as resp:
                    self.stats.record_check()

                    if resp.status == 429:
                        try:
                            data = await resp.json()
                            await asyncio.sleep(data.get("retry_after", 1.5) * 1.1)
                        except:
...
**Time:** `{datetime.now().strftime('%H:%M:%S')}`"""
    ...
    async def run(self):
        self.setup_signals()
        print("=" * 75)
        print("DISCORD USERNAME SNIPER v2.2 - CLEAN STATS")
        print("=" * 75)

        connector = aiohttp.TCPConnector(limit=CONCURRENCY * 3, ssl=False, keepalive_timeout=25)
        timeout = aiohttp.ClientTimeout(total=15, sock_connect=7)
